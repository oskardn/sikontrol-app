...
function vSetIp(vValue) {
	nIp = vValue;
}
function vSetPort(vValue) {
	nPort = vValue;
}

function vSetToken(vValue) {
	sToken = vValue;
}
...
async function vStoreData(vStorageKey, sValue) {
	try {
		...
		if (vValue) {
			switch (vStorageKey) {
				case "token":
					...
					Alert.alert("Succès", "Le token à bien été mis à jour", [
						{
							text: "OK",
						},
					]);
				...
				case "ip":
					...
					if (sValidIp.test(vValue) == true) {
						...
						Alert.alert("Succès", "L'IP à bien été mis à jour", [
							{
								text: "OK",
							},
						]);
				...
				case "port":
					if (vValue >= 1 && vValue <= 65535) {
						...
						Alert.alert("Succès", "Le port à bien été mis à jour", [
							{
								text: "OK",
							},
						]);
			...
			Alert.alert("Erreur", "Veuillez renseigner une valeur", [
				{
					text: "OK",
				},
			]);
...
const Sikontrol = ({ navigation }) => {
	...
	vGetData("ip").then((value) => {
		// console.log(value);

		nIp = value;
	});

	nIp = vGetData("port").then((value) => {
		nIp = value;
	});
